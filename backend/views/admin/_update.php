...
if (!$model->address) {
    echo Html::button('Add Address', ['value' => Url::to('/address/create?id=' . $model->id), 'class' => 'btn btn-success', 'id' => 'modalButton']);
} else {
...
<?= $form->field($model, 'password')->textInput(['maxlength' => true])->widget(PasswordInput::classname(), []) ?>
<?= $form->field($model, 'first_name')->textInput(['maxlength' => true]) ?>
<?= $form->field($model, 'last_name')->textInput(['maxlength' => true]) ?>
<?= $form->field($model, 'status')->dropDownList(
    [$model->status => $model->status, 'active' => 'active', 'inactive' => 'inactive', 'deleted' => 'deleted']
); ?>
